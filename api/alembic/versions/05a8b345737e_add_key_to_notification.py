...
def upgrade() -> None:
    """Upgrade schema."""
    notification_key_enum = sa.Enum('ROLES_CHANGED', 'DB_ACCESS_GIVEN', name='notificationkey')
    notification_key_enum.create(op.get_bind(), checkfirst=True)
    op.add_column(
        'notification',
        sa.Column('key', sa.Enum('ROLES_CHANGED', 'DB_ACCESS_GIVEN', name='notificationkey'), nullable=True),
        schema='app_data',
    )
