import { useProfileForm } from "@/features/userProfile/hooks/profile/useProfileForm";
...
function useProfileFormWithErrors(user = mockUser) {
  const hook = useProfileForm(user);
...
describe("useProfileForm", () => {
  ...
  it("skips the API call when no field has changed", async () => {
    ...
    const { result } = renderHookWithProviders(() => useProfileForm(mockUser));
  ...
  it("shows a validation error when username is too short", async () => {
    const { result } = renderHookWithProviders(() => useProfileFormWithErrors());
